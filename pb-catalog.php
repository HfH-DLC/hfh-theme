...
$plugin_assets = new Assets( 'pressbooks', 'plugin' );
...
<head>
	...
	?>
 	...
 	<link rel="stylesheet" type="text/css" href="<?php echo $plugin_assets->getPath( 'styles/style-catalog.css' ); ?>" />
    <link rel="stylesheet" type="text/css" href="<?php echo get_stylesheet_directory_uri() . '/css/pb-catalog.css' ; ?>" />
	...
	<script src="<?php echo $plugin_assets->getPath( 'scripts/matchheight.js' ); ?>" type="text/javascript"></script>
	<script src="<?php echo $plugin_assets->getPath( 'scripts/isotope.js' ); ?>" type="text/javascript"></script>
	<script src="<?php echo $plugin_assets->getPath( 'scripts/small-menu.js' ); ?>" type="text/javascript"></script>
	<script src="<?php echo $plugin_assets->getPath( 'scripts/catalog.js' ); ?>" type="text/javascript"></script>
		...
		<div class="log-wrap">	<!-- Login/Logout -->
			...
			<?php else : ?>
				...
				if ( get_current_user_id() == $pb_user_id || is_super_admin() ) {
					...
					?>
					<a href="<?php echo $admin_url; ?>"><?php _e( 'Admin', 'pressbooks' ); ?></a>
					<?php
				}
				?>
			<?php endif; ?>
	...
	<?php endif ?>
		...
		<div class="sidebar-inner-wrap">
			...
			if ( ! empty( $profile['pb_catalog_about'] ) ) {
				$profile['pb_catalog_about'] = \Pressbooks\HtmLawed::filter(
					$profile['pb_catalog_about'],
					array(
						'safe'           => 1,
			...
			<?php for ( $i = 1; $i <= 2; ++$i ) : ?>
				...
				<ul>
					<?php
					foreach ( $tags as $val ) {
						echo "<li><button class=\"filter-group-{$i}\" data-filter=\".{$val['id']}\">";
						echo "{$val['tag']}</button></li>\n";
					}
					?>
				</ul>
			<?php endfor; ?>
		...
		<div class="catalog-content" id="catalog-content">
			...
			<?php foreach ( $books as $b ) : ?>
				...
				<div class="book-data mix<?php echo _tag_classes( $b ); ?>">
					...
					<div class="book-info">
						<h2 class="book-title"><?php echo $b['title']; ?></h2>
						<p class="book-author"><?php echo $b['author']; ?></p>
					</div><!-- end book-info -->
