...
if (!function_exists('hfh_theme_posted_on')) :
	...
	function hfh_theme_posted_on()
	{
		...
		$posted_on = sprintf(
			...
			esc_html_x('%s', 'post date', 'hfh-theme'),
			$time_string
		);
	...
	function hfh_theme_entry_footer()
	{
		...
		if ('post' === get_post_type()) {
			...
			if ($categories_list) {
				...
				printf('<span class="cat-links">' . esc_html__('Posted in %1$s', 'hfh-theme') . '</span>', $categories_list); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
...
if (!function_exists('hfh_theme_categories')) :
	...
	function hfh_theme_categories()
	{
		...
		if ($categories_list) {
			/* translators: 1: list of categories. */
			printf('<span class="cat-links">' . esc_html__('%1$s', 'hfh-theme') . '</span>', $categories_list); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
